...
def run_sliding_classification_tuning_static(subjects, w_length, w_step, csp_components, c, kernel, gamma, degree):
    scores_across_subjects = []
    kappa_across_subjects = []
...
def hyperparameter_tuning (parameters_list, subjects):
    ...
    for n, param_set in enumerate(parameters_list):
        ...
        _, _, _, _, accuracy, _ = run_sliding_classification_tuning_static(subjects, w_length, w_step, csp_components, c, kernel, gamma, degree)
        ...
        #Write to file
        h = open(project_root + "/reports/figures/cumulative/SVM/dynamicVSstatic/sliding_model_tuning1.txt", "w")
...
def tune_svm_sliding():
    ...
    degree = best_params_sliding['degree']
    ...
    best_itr_tune, best_itr_patience, best_confidence_type = run_random_search(subjects, confidence_types, patience_values, threshold_values, w_length, w_step, sfreq, csp_components, c, kernel, gamma, degree)
