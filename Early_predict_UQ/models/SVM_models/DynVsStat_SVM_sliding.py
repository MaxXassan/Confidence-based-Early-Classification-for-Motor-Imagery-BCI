...
def run_sliding_classification_dynamic(subjects, threshold, patience, confidence_type, w_length, w_step, sfreq, csp_components, c , kernel, gamma, degree):
...
def run_sliding_classification_static(subjects, w_length, w_step, csp_components, c , kernel, gamma, degree, time):
    scores_across_subjects = []
    kappa_across_subjects = []
    itrs_across_subjects = []
...
def main_svm_sliding():
    ...
    for i, time in enumerate(prediction_time_dynamic):
        ...
        accuracy_stat, kappa_stat, _, subjects_accuracies_stat, mean_scores_across_subjects_stat, subjects_kappa_stat, mean_kappa_across_subjects_stat, subjects_itrs_stat, _,  cm_stat, csp_stat = run_sliding_classification_static(subjects, w_length, w_step, csp_components, c, kernel, gamma, degree, time)
        #accuracy and kappa are not single numbers here
        accuracy_static.append(accuracy_stat)
        kappa_static.append( kappa_stat,)
        accuracy_static_total.append(mean_scores_across_subjects_stat)
        kappa_static_total.append(mean_kappa_across_subjects_stat)
        _, _, _, _, _, _, itr = calculate_best_itr_dyn(best_itr = 0, accuracy = accuracy_stat, prediction_time = time, best_subjects_accuracies_dyn= None, best_subjects_prediction_times_dyn= None, best_subjects_kappa_dyn= None, best_subjects_itrs_dyn= None, best_cm_dyn= None, subjects_accuracies_dyn= None, subjects_prediction_times_dyn= None, subjects_kappa_dyn= None, subjects_itrs_dyn = None, cm_dyn = None)
