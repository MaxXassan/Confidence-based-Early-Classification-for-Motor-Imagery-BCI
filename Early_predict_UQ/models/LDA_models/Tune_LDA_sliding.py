            ...
            for epoch_idx in range(len(test_idx)):
                ...
                for n in w_start:
                    X_test_window = csp.transform(
                        train_data[test_idx][:, :, n : (n + w_length)]
                    )
...
def run_sliding_classification_tuning_static(
...
def create_parameterslist(sfreq):
    ...
    parameters = {
        ...
        "shrinkage": [None, "auto"]
        + list(
            np.round(rng.uniform(0.0, 1.0, 5), 2)
        ),  # None, 'auto', or float between 0 and 1
        "n_components": [None]
        + [
            1,
            2,
            3,
        ],  # Number of LDA components: Number of components (<= min(n_classes - 1, n_features)) -> 3
        "tol": [
            1.0e-2,
            1.0e-3,
            1.0e-4,
...
def hyperparameter_tuning(parameters_list, subjects):
    ...
    for n, param_set in enumerate(parameters_list):
        ...
        h = open(
            project_root
            + "/reports/figures/cumulative/LDA/dynamicVSstatic/sliding_model_tuning1.txt",
            "w",
        )
...
def tune_lda_sliding():
    ...
    best_params_sliding, _ = hyperparameter_tuning(
        parameters_list, subjects
    )  # tuned on accuracy as we dont have pred time for itr
    ...
    w_start = np.arange(
        0, epochs_info(length=True) - w_length, w_step
    )  # epochs_info(length= True) = 1001
