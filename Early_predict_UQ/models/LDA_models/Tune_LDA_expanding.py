...
def create_parameterslist(sfreq):
    ...
    parameters = {
        ...
        "initial_window_length": np.round(
            sfreq * np.round(rng.uniform(0.1, 1, 10), 1)
        ).astype(int),
        "expansion_rate": np.round(
            sfreq * np.round(rng.uniform(0.1, 0.5, 10), 1)
        ).astype(int),
...
def hyperparameter_tuning(parameters_list, subjects):
    ...
    for n, param_set in enumerate(parameters_list):
        ...
        h = open(
            project_root
            + "/reports/figures/cumulative/LDA/dynamicVSstatic/expanding_model_tuning1.txt",
            "w",
        )
...
def tune_lda_expanding():
    ...
    best_params_expanding, _ = hyperparameter_tuning(
    ...
    print(
        ...
        flush=True,
    )  # NB!- Confidence type is no longer a tuned parameter.
