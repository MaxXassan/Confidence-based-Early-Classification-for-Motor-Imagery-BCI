...
def run_expanding_classification_static(subjects, initial_window_length, expansion_rate, csp_components, pred_times):
    ...
    for person in subjects:
        ...
        X_train = spoc.fit_transform(train_data, train_labels)
        ...
        for n, window_start in enumerate(pred_times):
            ...
            X_test_window = spoc.transform(test_data[:, :, w_start[0]: window_length])
    ...
    # confusion matrix
    cm = np.divide(cm, number_cm)
    return subjects_accuracies, scores_across_subjects, subjects_kappa, kappa_across_subjects, subjects_itrs, itrs_across_subjects, accuracy, kappa, itr, cm, spoc
