...
def run_expanding_classification_dynamic(
    ...
    for person in subjects:
        ...
        epochs_data = epochs.get_data(copy=False)  # type: ignore
        ...
        test_indexes = [
            i
            for i, epoch in enumerate(epochs)
            if epochs[i].metadata["session"].iloc[0] == "1test"
        ]
        ...
        if solver == "svd":
            lda = LinearDiscriminantAnalysis(
                solver=solver, tol=tol, n_components=n_components
            )
        else:
            lda = LinearDiscriminantAnalysis(
                solver=solver, shrinkage=shrinkage, n_components=n_components
            )
        ...
        w_start = np.arange(
            0, epochs_data.shape[2] - initial_window_length, expansion_rate
        )
        # Testing/inference
        ...
        for epoch_idx in range(len(test_indexes)):
            ...
            for n, window_start in enumerate(w_start):
                ...
                X_test_window = csp.transform(
                    test_data[:, :, w_start[0] : window_length]
                )
                ...
                if predict:
                    # score
                    score = lda.score(
                        X_test_epoch_window.reshape(1, -1), [test_labels[epoch_idx]]
                    )
        ...
        _, _, _, _, _, _, itr = calculate_best_itr_dyn(
            best_itr=0,
            accuracy=np.mean(scores_across_epochs),
            prediction_time=np.mean(predict_time_across_epochs),
            ...
            best_cm_dyn=None,
            subjects_accuracies_dyn=None,
            subjects_prediction_times_dyn=None,
            subjects_kappa_dyn=None,
            subjects_itrs_dyn=None,
            cm_dyn=None,
        )
        ...
        if current_person == 1:
            ...
            prediction_time_across_subjects = np.vstack(
                (prediction_time_across_subjects, predict_time_across_epochs)
            )
    ...
    # prediction time
    mean_prediction_time_across_subjects = np.mean(
        prediction_time_across_subjects, axis=1
    )
    ...
    return (
        ...
        subjects_kappa,
        subjects_itrs,
        mean_scores_across_subjects,
        mean_kappa_across_subjects,
        mean_prediction_time_across_subjects,
        mean_itr_across_subjects,
    )


def run_expanding_classification_static(
    subjects,
    initial_window_length,
    expansion_rate,
    csp_components,
    ...
    for person in subjects:
        ...
        itrs_across_epochs = []
        w_start = np.arange(
            0, test_data.shape[2] - initial_window_length, expansion_rate
        )
        predictions_across_epochs = []
        window_length = np.round(np.array(time)).astype(int)
        for epoch_idx in range(len(test_indexes)):
...
# Q =  (60/T)
# ITR  = B * Q (bits/min)
def calculate_best_itr_dyn(
    best_itr,
    accuracy,
    prediction_time,
    best_subjects_accuracies_dyn,
    best_subjects_prediction_times_dyn,
    best_subjects_kappa_dyn,
    best_subjects_itrs_dyn,
    best_cm_dyn,
    subjects_accuracies_dyn,
    subjects_prediction_times_dyn,
    subjects_kappa_dyn,
    subjects_itrs_dyn,
    cm_dyn,
):
...
def plot_confusion_matrix(cm_stat, cm_dyn):
    ...
    s = sns.heatmap(
        ...
        fmt=".1f",
        cmap="magma",
        xticklabels=["Left hand", "Right hand", "Tongue", "Feet"],
        yticklabels=["Left hand", "Right hand", "Tongue", "Feet"],
    )
    s.set(xlabel="Predicted Label", ylabel="True Label")
    plt.savefig(
        project_root
        + "/reports/figures/cumulative/LDA/dynamicVSstatic/Expanding_dynamic_ConfusionMatrix.png"
    )
    ...
    plt.savefig(
        project_root
        + "/reports/figures/cumulative/LDA/dynamicVSstatic/Expanding_static_ConfusionMatrix.png"
    )
...
def main_lda_expanding():
    ...
    # best_params_expanding, best_itr_patience, _ = tune_lda_expanding()
    best_itr_patience = 5
    best_confidence_type = "neg_norm_shannon"

    # Values found through tuning - see Tune_(LDA/SVM)_(expanding/sliding).py
    ...
    solver = "lsqr"  # best_params_expanding['solver']
    n_components = 3  # best_params_expanding['n_components']
    tol = None  # best_params_expanding['tol']
    shrinkage = 0.14  # best_params_expanding['shrinkage']
    ...
    for n, threshold in enumerate(threshold_values):
        ...
        for i, subj_score_dyn in enumerate(mean_scores_across_subjects):
            _, _, _, _, _, _, itr_pr_subj_dyn = calculate_best_itr_dyn(
                best_itr=0,
                accuracy=mean_scores_across_subjects[i],
                prediction_time=mean_prediction_time_across_subjects[i],
                best_subjects_accuracies_dyn=None,
                best_subjects_prediction_times_dyn=None,
                best_subjects_kappa_dyn=None,
                best_subjects_itrs_dyn=None,
    ...
    print(
        "Evaluation (2):Finding static model accuracy, itr, and kappa for the given prediction times from the dynamic model"
    )
    ...
    for i, time in enumerate(prediction_time_dynamic):
        ...
        (
            ...
            mean_scores_across_subjects_stat,
            subjects_kappa_stat,
            mean_kappa_across_subjects_stat,
            subjects_itrs_stat,
            _,
            cm_stat,
            csp_stat,
        ) = run_expanding_classification_static(
            subjects,
            initial_window_length,
            expansion_rate,
            csp_components,
        ...
        for l, subj_score in enumerate(mean_scores_across_subjects_stat):
            _, _, _, _, _, _, itr_pr_subj_stat = calculate_best_itr_dyn(
                best_itr=0,
                accuracy=mean_scores_across_subjects_stat[l],
                prediction_time=prediction_time_dynamic_total[i][l],
                best_subjects_accuracies_dyn=None,
                best_subjects_prediction_times_dyn=None,
                best_subjects_kappa_dyn=None,
                best_subjects_itrs_dyn=None,
    ...
    prediction_time_dynamic_total = (
        np.array(prediction_time_dynamic_total) / sfreq + tmin
    )
    ...
    h = open(
        project_root
        + "/reports/figures/cumulative/LDA/dynamicVSstatic/expanding_model_optimal_parameters.txt",
        "w",
    )
    h.write(
        f"Chosen parameters: \n - csp: {csp_components}, \n - initial_window_length: {initial_window_length}\n - expansion-rate:  {expansion_rate} \n - confidence_type: {best_confidence_type}, \n - patience: {best_itr_patience} out of {patience_values}"
    ...
    subject_tuples = [
        (i + 1, acc) for i, acc in enumerate(np.mean(accuracy_dynamic_total, axis=0))
    ]
    sorted_subjects = sorted(subject_tuples, key=lambda x: x[1], reverse=True)
    f = open(
        project_root
        + "/reports/figures/cumulative/LDA/dynamicVSstatic/expanding_dynamic_model_accuracy_by_subject.txt",
        "w",
    )
    ...
    subject_tuples = [
        (i + 1, acc) for i, acc in enumerate(np.mean(accuracy_static_total, axis=0))
    ]
    sorted_subjects = sorted(subject_tuples, key=lambda x: x[1], reverse=True)

    f = open(
        project_root
        + "/reports/figures/cumulative/LDA/dynamicVSstatic/expanding_static_model_accuracy_by_subject.txt",
        "w",
    )
    ...
    subject_tuples = [
        (i + 1, acc) for i, acc in enumerate(np.mean(kappa_dynamic_total, axis=0))
    ]
    sorted_subjects = sorted(subject_tuples, key=lambda x: x[1], reverse=True)
    f = open(
        project_root
        + "/reports/figures/cumulative/LDA/dynamicVSstatic/expanding_dynamic_model_kappa_by_subject.txt",
        "w",
    )
    ...
    subject_tuples = [
        (i + 1, acc) for i, acc in enumerate(np.mean(kappa_static_total, axis=0))
    ]
    sorted_subjects = sorted(subject_tuples, key=lambda x: x[1], reverse=True)

    f = open(
        project_root
        + "/reports/figures/cumulative/LDA/dynamicVSstatic/expanding_static_model_kappa_by_subject.txt",
        "w",
    )
    ...
    subject_tuples = [
        (i + 1, acc) for i, acc in enumerate(np.mean(itr_dynamic_total, axis=0))
    ]
    sorted_subjects = sorted(subject_tuples, key=lambda x: x[1], reverse=True)
    f = open(
        project_root
        + "/reports/figures/cumulative/LDA/dynamicVSstatic/expanding_dynamic_model_itrs_by_subject.txt",
        "w",
    )
    ...
    subject_tuples = [
        (i + 1, acc) for i, acc in enumerate(np.mean(itr_static_total, axis=0))
    ]
    sorted_subjects = sorted(subject_tuples, key=lambda x: x[1], reverse=True)

    f = open(
        project_root
        + "/reports/figures/cumulative/LDA/dynamicVSstatic/expanding_static_model_itrs_by_subject.txt",
        "w",
    )
    ...
    subject_tuples = [
        (i + 1, acc)
        for i, acc in enumerate(np.mean(prediction_time_dynamic_total, axis=0))
    ]
    sorted_subjects = sorted(subject_tuples, key=lambda x: x[1], reverse=True)
    f = open(
        project_root
        + "/reports/figures/cumulative/LDA/dynamicVSstatic/expanding_dynamic_model_predtime_by_subject.txt",
        "w",
    )
    f.write(
        f"Average prediction time - Dynamic Model: {np.mean(prediction_time_dynamic)}\n"
    )
    ...
    sem_accuracy_dynamic = [
        np.std(scores) / np.sqrt(len(scores))
        for scores in np.array(accuracy_dynamic_total)
    ]  # (accuracy_dyn_tot: shape(thresholds, test_epochs)) - accuracies over x threshold, 288 epochs, averaged across the subjects. Finding SEM for each threshold value
    sem_pred_time_dynamic = [
        np.std(times) / np.sqrt(len(times))
        for times in np.array(prediction_time_dynamic_total)
    ]
    sem_accuracy_stat = [
        np.std(scores) / np.sqrt(len(scores))
        for scores in np.array(accuracy_static_total)
    ]  # (scores_across_subjects_stat: (subjects, predtimes)) - 9 subjects, statically for each given prediction time from the dyn model(correlates to a specific threshold value)
    ...
    plt.errorbar(
        prediction_time_dynamic,
        accuracy_dynamic,
        xerr=sem_pred_time_dynamic,
        yerr=sem_accuracy_dynamic,
        label="Dynamic model",
        fmt="o",
        color="blue",
        ecolor="black",
    ...
    plt.savefig(
        project_root
        + "/reports/figures/cumulative/LDA/dynamicVSstatic/ExpandingAccuracy.png"
    )
    plt.show()
    ...
    sem_kappa_dynamic = [
        np.std(scores) / np.sqrt(len(scores))
        for scores in np.array(kappa_dynamic_total)
    ]
    sem_kappa_stat = [
        np.std(scores) / np.sqrt(len(scores)) for scores in np.array(kappa_static_total)
    ]
    plt.figure(figsize=(8, 6))
    plt.style.use("ggplot")
    plt.xlabel("Prediction time (sec)")
    ...
    plt.savefig(
        project_root
        + "/reports/figures/cumulative/LDA/dynamicVSstatic/ExpandingKappa.png"
    )
    plt.show()

    # plot itrs
    sem_itr_dynamic = [
        np.std(scores) / np.sqrt(len(scores)) for scores in np.array(itr_dynamic_total)
    ]
    sem_itr_stat = [
        np.std(scores) / np.sqrt(len(scores)) for scores in np.array(itr_static_total)
    ]
    plt.figure(figsize=(8, 6))
    plt.style.use("ggplot")
    plt.xlabel("Prediction time (sec)")
    ...
    plt.savefig(
        project_root
        + "/reports/figures/cumulative/LDA/dynamicVSstatic/ExpandingItr.png"
    )
    plt.show()
